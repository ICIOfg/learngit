...
{
//1, 读取解析配置文件
//读取出错，完全可以忽略，可以定义一个缺省策略或使用系统缺省策略
...
//
	...
	{
		std::cout << "Configure Problem" << f.what() << endl;
		return -1;
	}
//2， 实例化Category对象
//		这些对象即使配置文件没有定义也可以使用，不过其属性继承其父Category
//		通常使用引用可能不太方便，可以使用指针，以后作指针使用
// 
...
// sub4 can not found in the config file, so the root category's appender and layout are used
